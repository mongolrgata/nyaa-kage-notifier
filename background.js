/**
 * Created by mongolrgata on 16.02.2015.
 */

function dummy() {
    console.log(arguments);
}

function reset() {
    chrome.storage.local.clear();
}

function getWatchList(callback) {
    chrome.storage.local.get('watchList', function (result) {
        var watchList = JSON.parse(result.watchList || '{}') || {};

        callback.call(watchList);
    });
}

function setAlarm() {
    var
        searchParams = {
            page   : 'search',
            cats   : '0_0',
            filter : '0',
            term   : ''
        },
        downloadParams = {
            page : 'download',
            tid  : ''
        };

    chrome.alarms.create('checkUpdate', {
        when            : Date.now(),
        periodInMinutes : 15
    });

    chrome.alarms.onAlarm.addListener(function (alarm) {
        switch (alarm['name']) {
            case 'checkUpdate':
                chrome.storage.local.get('watchList', function (result) {
                    var watchList = JSON.parse(result.watchList || '{}') || {};

                    function check(url, topStackStorage) {
                        $.get(
                            url,
                            function (data) {
                                var topStackCurrent = $(data).find(".tlist .tlistrow:first .tlistname").text();
                                var urlTop = $(data).find(".tlist .tlistrow:first .tlistname a").attr('href');

                                if (topStackCurrent !== topStackStorage) {

                                    chrome.notifications.create(
                                        urlTop,
                                        {
                                            type    : 'basic',
                                            iconUrl : 'data:image/jpeg;base64,',
                                            title   : '[New Record]',
                                            message : topStackCurrent
                                        },
                                        dummy
                                    );
                                }

                                watchList[url] = topStackCurrent;
                                chrome.storage.local.set({'watchList' : JSON.stringify(watchList)}, dummy);
                            }
                        );
                    }

                    for (var key in watchList) {
                        if (watchList.hasOwnProperty(key)) {
                            check(key, watchList[key]);
                        }
                    }
                    //var topStackStorage = result['topStack'];
                    //
                    //$.get(
                    //    'http://www.nyaa.se/',
                    //    function (data) {
                    //        var topStackCurrent = $(data).find(".tlist .tlistrow:first .tlistname").text();
                    //
                    //        // Firefox only
                    //        //var tid = $(data).find(".tlist .tlistrow:first .tlistname a")[0].searchParam.get('tid');
                    //
                    //        if (topStackCurrent !== topStackStorage) {
                    //            searchParams['term'] = topStackCurrent;
                    //            //downloadParams['tid'] = tid;
                    //
                    //            chrome.notifications.create(
                    //                'http://www.nyaa.se/?' + $.param(searchParams),
                    //                {
                    //                    type    : 'basic',
                    //                    iconUrl : 'data:image/jpeg;base64,',
                    //                    title   : '[New Record]',
                    //                    message : topStackCurrent
                    //                },
                    //                dummy
                    //            );
                    //        }
                    //
                    //        chrome.storage.local.set({'topStack' : topStackCurrent});
                    //    }
                    //);
                });
                break;
        }
    });

    chrome.notifications.onClicked.addListener(function (notificationId) {
        chrome.tabs.create({
            url : notificationId
        });

        // TODO I'm not sure about this
        chrome.windows.getCurrent(function (window) {
            var windowId = window['id'];

            chrome.windows.update(windowId, {focused : true}, dummy);
        });

        chrome.notifications.clear(notificationId, dummy);
    });
}

chrome.runtime.onInstalled.addListener(function () {
    setAlarm();
});
chrome.runtime.onStartup.addListener(function () {
    setAlarm();
});

// TODO Some magic here... i dunno lol
chrome.runtime.onInstalled.addListener(function () {
    //var my_id = new ImageData();
    //my_id.data = "data:image/png;base64,";

    chrome.declarativeContent.onPageChanged.removeRules(undefined, function () {
        //var cnv = $('<canvas/>');
        //var ctx = cnv[0].getContext("2d");
        ////var imgData = ctx.createImageData(19, 19);
        //var image = new Image();
        //image.src = "data:image/png;base64,";
        //ctx.drawImage(image, 0, 0);
        //var imgData = ctx.getImageData(0,0,19,19);

        chrome.declarativeContent.onPageChanged.addRules([
            {
                conditions : [
                    new chrome.declarativeContent.PageStateMatcher({
                        pageUrl : {queryContains : 'horrible+saekano+1080p'}
                    })
                ],
                actions    : [new chrome.declarativeContent.ShowPageAction()]
            }/*,
             {
             conditions : [
             new chrome.declarativeContent.PageStateMatcher({
             pageUrl : {urlPrefix : 'https://osu.ppy.sh/'}
             })
             ],
             //actions    : [new chrome.declarativeContent.ShowPageAction()]
             actions    : [new chrome.declarativeContent.ShowPageAction(), new chrome.declarativeContent.SetIcon({imageData:imgData})]
             }*/
        ]);
    });
});

chrome.pageAction.onClicked.addListener(function () {
    var tablink = arguments[0].url;
    chrome.storage.local.get('watchList', function (result) {
        var watchList = JSON.parse(result.watchList || '{}') || {};

        if (!(tablink in watchList)) {
            watchList[tablink] = $('body').find(".tlist .tlistrow:first .tlistname").text();

            chrome.storage.local.set({'watchList' : JSON.stringify(watchList)}, dummy);
        }
    });
});

chrome.webNavigation.onCompleted.addListener(function (details) {
    var id = details.tabId;
    var url = details.url;

    chrome.storage.local.get('watchList', function (res) {
        var wl = JSON.parse(res.watchList || '{}') || {};

        if (url in wl) {
            chrome.pageAction.setIcon({
                tabId : id,
                path  : "images/38_2.png"
            }, dummy);
        }
        else {
            chrome.pageAction.setIcon({
                tabId : id,
                path  : "images/38.png"
            }, dummy);
        }
    });
});
